...
## Описание работы:
БД состоит из двух таблиц (one-to-one relationship)- пользователей и их зарплат.
При первом запуске приложения в таблице пользователей будет создан аккаунт работодателя с правами админа, который позволяет редактировать записи сотрудников в таблице зарплат. Учетные данные аккаунта админа задаются в **.env**-файле в виде переменных:
...
1. Регистрация нового сотрудника в системе - при этом:
  - создается аккаунт сотрудника в таблице пользователей
  ...
  - высылается сообщение админу о регистрации нового сотрудника и необходимости заполнения его записи в таблице зарплат (TODO)
2. Заполнение работодателем/админом записи зарплаты сотрудника.
3. Авторизация сотрудника в системе - получение токена.
4. Просмотр текущей зарплаты и даты следующего повышения возможен только для авторизованного пользователя по адресу 'http://localhost:8000/api/v1/salary/my_salary'
